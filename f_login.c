...
void do_register_exp(struct fetion_account_data *sip, gint expire)
{
	...
	body = g_strdup_printf(" <args><device type=\"PC\" version=\"0\" client-version=\"3.1.0480\" /><caps value=\"fetion-im;im-session;temp-group\" /><events value=\"contact;permission;system-message\" /><user-info attributes=\"all\" /><presence><basic value=\"400\" desc=\"\" /></presence></args>");
	...
	else if(sip->registerstatus == FETION_REGISTER_RETRY && (sip->registrar.digest_session_key) )
	{
...
void RetriveSysCfg_cb(gpointer sodata, gint source, const gchar *error_message)
{
	...
	if(rcv_len>0)
	{
		...
		purple_debug_info("fetion","systemconfig:after servers[%s]",sip->SysCfg.buf);
		item = xmlnode_get_child(son_node,"sipc-proxy");
...
gint RetriveSysCfg(gpointer sodata, gint source, const gchar *error_message)
{
	...
	if(sip->mobileno!=NULL)
	{
		body = g_strdup_printf("<config><user mobile-no=\"%s\" /><client type=\"PC\" version=\"3.1.0480\" platform=\"W5.1\" /><servers version=\"%s\" /><service-no version=\"%s\" /><parameters version=\"%s\" /><hints version=\"%s\" /><http-applications version=\"%s\" /><client-config version=\"%s\" /></config>\r\n\r\n",sip->mobileno,sip->ServerVersion,sip->ServiceNoVersion,sip->ParaVersion,sip->HintsVersion,sip->HttpAppVersion,sip->ClientCfgVersion);
	}
	else
	{
		body = g_strdup_printf("<config><user sid=\"%s\" /><client type=\"PC\" version=\"3.1.0480\" platform=\"W5.1\" /><servers version=\"%s\" /><service-no version=\"%s\" /><parameters version=\"%s\" /><hints version=\"%s\" /><http-applications version=\"%s\" /><client-config version=\"%s\" /></config>\r\n\r\n",sip->username,sip->ServerVersion,sip->ServiceNoVersion,sip->ParaVersion,sip->HintsVersion,sip->HttpAppVersion,sip->ClientCfgVersion);
	}
...
void fetion_login(PurpleAccount *account)
{
	...
	sip->registertimeout = purple_timeout_add( 60000,(GSourceFunc) LoginToSsiPortal,sip);

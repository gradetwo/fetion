...
static void fetion_set_status(PurpleAccount *account, PurpleStatus *status)
{
	...
	body = g_strdup_printf("<args><presence><basic value=\"%d\" /></presence></args>",status_code);
	send_sip_request(account->gc,"S","","","N: SetPresence\r\n",body,NULL,NULL);
	g_free(body);
	//<args><presence><basic value="400" /></presence></args>
}

static struct sip_connection *connection_find(struct fetion_account_data *sip, int fd)
{
...
static struct sip_connection *connection_create(struct fetion_account_data *sip, int fd)
{
...
static GList *fetion_status_types(PurpleAccount *acc)
{
	...
	status = purple_status_type_new_full(PURPLE_STATUS_AVAILABLE,
			"available", NULL, FALSE, TRUE, FALSE);
	...
	status = purple_status_type_new_full(PURPLE_STATUS_MOBILE,
			"mobile", _("Mobile on Line"), FALSE, TRUE, FALSE);
	types = g_list_append(types, status);
...
static void fetion_canwrite_cb(gpointer data, gint source, PurpleInputCondition cond)
{
...
static void send_later_cb(gpointer data, gint source, const gchar *error)
{
	...
	conn->inputhandler = purple_input_add(sip->fd, PURPLE_INPUT_READ, fetion_input_cb, gc);
}
	...
	if (ret < writelen)
	{
		if(!sip->tx_handler)
			sip->tx_handler = purple_input_add(sip->fd,
					PURPLE_INPUT_WRITE, fetion_canwrite_cb,
					gc);
...
static void transactions_remove(struct fetion_account_data *sip, struct transaction *trans)
{
...
static void transactions_add_buf(struct fetion_account_data *sip, const gchar *buf, void *callback)
{
	...
	if(!strcmp(trans->msg->method, "M"))
		trans->timer = purple_timeout_add(60000, (GSourceFunc) transaction_timeout, trans);
...
static struct transaction *transactions_find(struct fetion_account_data *sip, struct sipmsg *msg)
{
	struct transaction *trans;
	...
	if (cseq)
	{
		...
		purple_debug(PURPLE_DEBUG_MISC, "fetion", "Received message contains no CSeq header.\n");
	}
...
static void fetion_unsubscribe(char *name, struct fetion_buddy *buddy, struct fetion_account_data *sip)
{
	if (buddy->dialog)
	{
...
static int fetion_im_send(PurpleConnection *gc, const char *who, const char *what, PurpleMessageFlags flags)
{
	struct fetion_account_data *sip = gc->proto_data;
...
gboolean process_register_response(struct fetion_account_data *sip, struct sipmsg *msg, struct transaction *tc)
{
	...
	purple_debug(PURPLE_DEBUG_MISC, "fetion", "in process register response response: %d\n", msg->response);
	switch (msg->response)
	{
		case 200:
			if(sip->registerstatus < FETION_REGISTER_COMPLETE)
			{
				...
				sip->GetContactTimeOut = purple_timeout_add(5000,(GSourceFunc)GetContactList,sip); 
				GetContactList(sip);
			}
		...
		default:
			if (sip->registerstatus != FETION_REGISTER_RETRY)
			{
				purple_debug_info("fetion", "Unrecognized return code for REGISTER.%d\n",msg->response);
				if (sip->registrar.retries > FETION_REGISTER_RETRY_MAX)
				{
					purple_connection_error_reason(sip->gc,
...
static void process_incoming_notify(struct fetion_account_data *sip, struct sipmsg *msg)
{
	gchar *from;
	const gchar *fromhdr;
	...
	if(!pidf)
	{
		purple_debug_info("fetion", "process_incoming_notify: no parseable pidf\n");
		...
		if (sshdr)
		{
			...
			while (ssparts[i])
			{
				...
				{
					purple_debug_info("fetion", "Subscription expired!");
	...
	if(!basicstatus)
	{
		purple_debug_info("fetion", "process_incoming_notify: no basic found\n");
		xmlnode_free(pidf);
		g_free(from);
		return;
	}
	...
	if(!basicstatus_data)
	{
		purple_debug_info("fetion", "process_incoming_notify: no basic data found\n");
		xmlnode_free(pidf);
		g_free(from);
		return;
	}
...
static void process_input_message(struct fetion_account_data *sip, struct sipmsg *msg)
{
	...
	{ /* request */
		...
		if(trans)
		{
			if(msg->response == 407)
			{
				...
				{
					purple_debug_info("fetion:","AddBuddy:522\n");
					if(trans->callback)
						(trans->callback)(sip, msg, trans);
				}
				found = TRUE;
			} else if(msg->response == 406 ||msg->response == 480 || msg->response == 400 )
			{
				//406 Not Acceptable
				purple_connection_error_reason(sip->gc,
						PURPLE_CONNECTION_ERROR_NETWORK_ERROR,	
						_("Error Connect.Try reconnect."));
			}
			else if((!strcmp(trans->msg->method,"M")) && (msg->response==200 ||msg->response==280))
			{
				...
				if(msg->response == 100)
				{
					...
					purple_debug_info("fetion", "got trying response\n");
				} else {
					sip->proxy.retries = 0;
			...
			purple_debug(PURPLE_DEBUG_MISC, "fetion", "received response to unknown transaction");
		}
	}
	if(!found)
	{
		purple_debug(PURPLE_DEBUG_MISC, "fetion", "received a unknown sip message with method %s and response %d\n", msg->method, msg->response);
	}
}

static void process_input(struct fetion_account_data *sip, struct sip_connection *conn)
{
	char *cur;
	char *dummy;
	struct sipmsg *msg;
	...
	do
	{
		...
		if((cur = strstr(conn->inbuf, "\r\n\r\n")) != NULL)
		{
			...
			purple_debug(PURPLE_DEBUG_MISC, "fetion", "in process response response: %d\n", msg->response);
			process_input_message(sip, msg);
		} 
		else {
			purple_debug(PURPLE_DEBUG_MISC, "fetion", "received a incomplete sip msg: %s\n", conn->inbuf);
			break;
		}
...
void srvresolved( gpointer data)
{
	...
	if(!sip->realport) sip->realport = 8080;
...
static void fetion_close(PurpleConnection *gc)
{
	...
	if(sip)
	{
		...
		if(sip->GetContactTimeOut) purple_timeout_remove(sip->GetContactTimeOut);
	}
...
static GList * fetion_buddy_menu(PurpleBuddy *buddy)
{
	...
	static GList *
fetion_blist_node_menu(PurpleBlistNode *node)
{
...
static void * fetion_find_group_with_id(struct fetion_account_data *sip, int chat_id)
{
	GList *l; 
...
static void fetion_chat_invite(PurpleConnection *gc, int id, const char *msg,const char *who)
{
...
static int  fetion_chat_send(PurpleConnection *gc, int id, const char *message, PurpleMessageFlags flags)
{
	struct fetion_account_data *sip = gc->proto_data;
	...
	send_sip_request(sip->gc, "M", NULL, fullto, hdr, text, dialog, (TransCallback) SendMsgTimeout_cb);
	serv_got_chat_in(sip->gc, id,purple_account_get_alias(sip->account), 0,
			message, time(NULL));
...
static void fetion_get_info(PurpleConnection *gc, const char *who)
{
	...
	static GList *
fetion_attention_types(PurpleAccount *account)
{
...
fetion_send_attention(PurpleConnection *gc, const char *who, guint type)
{
	...
	*/
	...
	send_sip_request(sip->gc, "IN", NULL, fullto, NULL, msg, buddy->dialog, NULL);
	g_free(msg);
	g_free(fullto);
	...
	static PurpleCmdRet
fetion_cmd_nudge(PurpleConversation *conv, const gchar *cmd, gchar **args, gchar **error, void *data)
{

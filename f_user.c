...
gboolean GetPersonalInfo_cb(struct fetion_account_data *sip, struct sipmsg *msg, struct transaction *tc) 
{
	xmlnode *root,*personal;
	const gchar *nickname;
	const gchar *impresa;
	purple_debug(PURPLE_DEBUG_MISC, "fetion", "in process GetPersonalInfo response response: %d\n", msg->response);
	switch (msg->response)
	{
		case 200:
...
gboolean GetPersonalInfo(struct fetion_account_data *sip)
{
	...
	body = g_strdup("<args><personal attributes=\"all\" /><services version=\"\" attributes=\"all\" /><config version=\"0\" attributes=\"all\" /><mobile-device attributes=\"all\" /></args>");

	send_sip_request(sip->gc, "S", "","", hdr, body, NULL,GetPersonalInfo_cb);

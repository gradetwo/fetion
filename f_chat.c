...
void fetion_send_message(struct fetion_account_data *sip, const gchar *to, const gchar *msg, const gchar *type, const gboolean sms)
{
	gchar *hdr;
	gchar *fullto;
	...
	send_sip_request(sip->gc, "M", NULL, fullto, hdr, msg, buddy->dialog,(TransCallback) SendMsgTimeout_cb );
	g_free(hdr);
	g_free(fullto);
}
